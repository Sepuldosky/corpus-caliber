...
1. **Namespace: tabla única registrada (cita COR-2).** Cada archivo **que consume la tabla del módulo** la cachea vía `local CALIBER = Corpus.GetModule("caliber")` (el init la registró antes vía `Corpus.RegisterModule`; el stool la resuelve lazy dentro de funciones); los que no la consumen —`shared`, `client_options`— no la piden. **Ningún archivo declara un global `ADS`, `Caliber` ni `CALIBER` suelto** — ni tablas auxiliares globales (el browser y los FX de escudo usan file-locals, no globals). Depende del invariante by-ref del registro de Corpus (§3, §11 de la arquitectura).
